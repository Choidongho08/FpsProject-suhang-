...
namespace _02_Code.Core.DI
{
    ...
    public class Injector : MonoBehaviour
    {
        ...
        private void Awake()
        {
            //인터페이스를 구현한 모든 녀석들 중 Provide 어트리뷰트가 있는 녀석을 찾아서 딕셔너리에 넣는다.
            IEnumerable<IDIProvider> providers = FindMonoBehaviours().OfType<IDIProvider>();
            foreach (IDIProvider pro in providers)
            {
        ...
        private void Inject(MonoBehaviour mono)
        {
            ...
            {
                //파라메터의 타입정보만 가져온다.
                Type[] requireParam = method.GetParameters().Select(p => p.ParameterType).ToArray();
        ...
        private void RegisterProvider(IDIProvider pro)
        {
            ...
            // 메서드에 Provide 되는 경우는 메서드들 다 가져와서 실행 시키고 가져옴
